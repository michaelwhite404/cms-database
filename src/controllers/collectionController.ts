...
import { notReservedField } from "../utils/validations";
...
export const validDatabase = catchAsync(
  async (
    ...
    if (!req.body.database)
      return next(
        new AppError("Database ID is required but is not present", 400)
      );
...
export const createCollection = catchAsync(
  async (
    ...
    if (!database) {
      return next(
        new AppError(
          "A collection must be added to a valid database you have access to.",
          400
        )
      );
    }
    ...
    if (bodyFields) {
      ...
      if (pNameIndex.length > 1)
        return next(
          new AppError("You cannot have multiple Primary Names", 400)
        );
      ...
      // Store Slug and Order Number
      for (let i = 0; i < bodyFields.length; i++) {
        const field = bodyFields[i];
        // If the field name is a reserved field, throw error
        if (!notReservedField(field.name))
          return next(new AppError(`'${field.name}' is a reserved name.`, 400));
        // If field type is 'User', throw error
        if (field.type === "User")
          return next(
            new AppError("User cannot set field as type 'User'.", 400)
          );
        fields.push(
